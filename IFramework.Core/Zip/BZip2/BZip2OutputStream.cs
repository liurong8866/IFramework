...
namespace IFramework.Core.Zip.BZip2
{
    ...
    public class BZip2OutputStream : Stream
    {
        ...
        private readonly char[] selector = new char[BZip2Constants.MAXIMUM_SELECTORS];
        private readonly char[] selectorMtf = new char[BZip2Constants.MAXIMUM_SELECTORS];
        ...
        private readonly int[] mtfFreq = new int[BZip2Constants.MAXIMUM_ALPHA_SIZE];
        ...
        */
        ...
        /// </summary>
        public override long Position { get => baseStream.Position; set => throw new NotSupportedException("BZip2OutputStream position cannot be set"); }
                ...
                if (j == 0) { zPend++; } else {
                    if (zPend > 0) {
                        ...
                        while (true) {
                            switch (zPend % 2) {
                                case 0:
                                    szptr[wr] = BZip2Constants.RUN_A;
                                    wr++;
                                    mtfFreq[BZip2Constants.RUN_A]++;
                                    break;
                                case 1:
                                    szptr[wr] = BZip2Constants.RUN_B;
                                    wr++;
                                    mtfFreq[BZip2Constants.RUN_B]++;
                                    break;
                            }
        ...
        private static void HbMakeCodeLengths(char[] len, int[] freq, int alphaSize, int maxLen)
        {
            ...
            bool tooLong;
            int[] heap = new int[BZip2Constants.MAXIMUM_ALPHA_SIZE + 2];
            ...
            while (true) {
                ...
                while (nHeap > 1) {
                    ...
                    weight[nNodes] = (int)((weight[n1] & 0xffffff00) + (weight[n2] & 0xffffff00)) | (1 + ((weight[n1] & 0x000000ff) > (weight[n2] & 0x000000ff) ? weight[n1] & 0x000000ff : weight[n2] & 0x000000ff));
                    parent[nNodes] = -1;
                    nHeap++;
                    heap[nHeap] = nNodes;
                    zz = nHeap;
