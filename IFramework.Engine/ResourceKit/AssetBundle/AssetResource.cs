    ...
    public class AssetResource : AbstractResource
    {
        ...
        public override bool Load()
        {
            ...
            if (Platform.IsSimulation && !assetName.Equals("assetbundlemanifest")) {
                ...
                // 根据包名+资源名获取资源路径
                string[] assetPaths = PlatformEnvironment.Instance.GetAssetPathsFromAssetBundleAndAssetName(resource.AssetName, assetName);
                ...
                obj = AssetType != null ? PlatformEnvironment.Instance.LoadAssetAtPath(assetPaths[0], AssetType) : PlatformEnvironment.Instance.LoadAssetAtPath<Object>(assetPaths[0]);
            }
            else {
                using ResourceSearcher searcher = ResourceSearcher.Allocate(assetBundleNameConfig, null, typeof(AssetBundle));
        ...
        public override IEnumerator LoadAsync(Action callback)
        {
            ...
            if (Platform.IsSimulation && !assetName.Equals("assetbundlemanifest")) {
                ...
                UnHoldDependResource();
                if (AssetType != null) {
                    asset = PlatformEnvironment.Instance.LoadAssetAtPath(assetPaths[0], AssetType);
                }
                else {
                    asset = PlatformEnvironment.Instance.LoadAssetAtPath<Object>(assetPaths[0]);
                }
            }
            else {
